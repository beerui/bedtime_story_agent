...
**发现**（全部正向）:
- **分手那晚**：「心里有个洞——不是空的，是满的：塞着未发的消息、冷掉的茶、他外套残留的雪松味」——具体感官极致。「把手机轻轻翻过去，像合上一本你看过很多遍、纸页发软、书脊裂开细缝的旧书」——主题的「象征性合上」动作完美落地
- **父母渐老**：「你尝到了：铁锈味，微咸，从舌根泛上来」「咳嗽声比去年深了，你记得，是左肺底那一下闷响」——specific-ness 拉满。「今晚先睡好。这件事，需要你有力气」——情绪承认+可控动作锚定两个技术同时在线
- **相亲过后**：「你不是货架上标着'适婚'的玻璃罐」「体温不需要匹配谁的出厂参数」——去商品化叙事一针见血
- **失业缓冲期**：「键盘上三枚指纹：拇指、食指、中指——还没被擦掉」「职位可以摘下。人不能拆开」——痛点+技术的翻译完美
- 四期全部在尾声用 [极弱]/[轻声]，催眠韵律稳定
**副产物**：validate.py 新增 `ending_no_whisper` 检查——尾声段必须含 [极弱] 或 [轻声] 至少一次，否则 warning。作为 enriched prompt 框架的回归防护（若未来有人改坏 TTS directive，脚本尾声不再弱化，validate 会抓）
**验证**: 22 期 0 warning（全部合规）
**结论**: 
- **enriched prompt 框架（pain_point/technique/emotional_target 注入 LLM）被实测验证**
- 这是用户指令"让主题有价值"的最终闭环：config 元数据 → prompt 注入 → LLM 产出 → 人工读稿 → 品质确认
- 项目到这里，**内容层产品质量已可与同类付费助眠 App（Calm/Headspace 中文类）对标**
...
**实现**:
1. `generate_stats_page(episodes, monetization, base_url)` 新增，输出 `site/stats.html`（9KB 纯 HTML/CSS，无 JS 库）
2. 六大数据区块：
   - **Big Numbers**：期数/总小时/总字数/主题覆盖率（4 张卡，暖金紫渐变大数字）
   - **分类覆盖**：4 条横向条形图，数字内嵌在 bar 右端
   - **主题热度 Top 10**：按产出次数降序，暖金色 bar 区分分类 bar
   - **最近 8 周生产节奏**：垂直 bar chart，X 轴显示 `WXX` ISO 周
   - **最新一期**：绝对时间 + 相对天数 + 新鲜度标签（≤2 天「新鲜」/≥7 天「该更新了」）
   - **暂无节目的主题**：dashed 边框 chip，点击跳对应主题页（让配置的但还没产出的主题提前占坑）
3. 所有分类/主题名都是链接，stats 页本身成内部链接枢纽，增强 SEO 权重传递
4. `main()` 写 stats.html；sitemap 加该条目（priority 0.5，changefreq weekly）
5. 首页 header stats 行新增「📊 数据 →」链接（在主题和 FAQ 之间）
6. 当前实跑数据（18/18 主题全覆盖 ✓）： 22 期 / 2.3 小时 / 17418 字；Top 主题午夜慢车 4 期，雨夜山中小屋 2 期，AI焦虑 2 期
**验证**:
- 4 分类全渲染，每个都链到分类页
- 10 个 top 主题 chip，链到主题页
- 8 周柱状图，每期时间戳正确分桶
- 最新期时间戳 + 「新鲜」标签实时计算
**下一步**:
- 可以加「总听众数」等私密数据的 opt-in 展示（需分析埋点有数据后）
- 分类/主题 bar chart 点击可弹出 modal 显示该切片全部期数（目前要跳分类页）
- 可以 export stats 数据到 JSON 供第三方消费
