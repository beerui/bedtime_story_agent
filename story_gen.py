...
def generate_story(theme_name: str, output_dir: str, target_words: int, extra_prompt: str = "") -> str:
    ...
    final_story = _llm_call(
        ...
        "【价值密度要求 — 让内容真正有疗愈价值】：\n"
        "- 每 200 字至少 1 个身体部位锚点（肩膀/后颈/胸口/太阳穴/眼皮/手指/脚趾/脊椎/腹部/额头/锁骨）\n"
        "- 引入段必须有 2+ 个情绪命名词（累/怕/慌/焦虑/紧张/疲惫/烦躁/孤独等），精确命名比模糊安慰更有力\n"
        "- 尾声段至少 3 个心理许可句式（允许/可以/不必/不用/不需要/没关系/不是你的错）\n"
        "- 全文至少 5 处呼吸引导（吸气/呼气/呼吸/吸/呼/气流）\n"
        "- 比喻/隐喻不少于 4 处（像/仿佛/如同），用具体物体而非抽象概念\n\n"
        f"初稿：\n{draft}",
        "3/润色",
    )
