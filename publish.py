...
def generate_rss(episodes: list[dict], base_url: str,
    ...
    for idx, ep in enumerate(episodes):
        ...
        # Per-episode square cover (1400x1400) — Apple Podcasts / 小宇宙 show
        # this in now-playing and episode list views. Falls back to the
        # channel-level cover when episode-specific square art isn't available.
...
def _build_newsletter_form(m: dict, context: str = "page") -> str:
    ...
    # Privacy policy link — context-aware relative path
    ...
    <section class="newsletter" aria-labelledby="{form_id}-title">
      ...
      <p class="nl-consent">点击订阅即同意 <a href="{privacy_prefix}privacy.html">隐私政策</a>——我们只发新节目通知，不出售邮箱，可随时退订。</p>
    </section>""")
...
def _legal_page_template(title: str, body_html: str, monetization: dict, base_url: str,
    ...
    """Shared template for privacy.html / terms.html — legal pages with
    consistent styling, breadcrumb, and footer back-links."""
    m = monetization or {}
    site_url = (base_url or m.get("site_url") or "").rstrip("/")
...
def generate_privacy_page(monetization: dict, base_url: str) -> str:
    """Privacy policy — covers what data is collected, how it's used, no sale,
    contact + opt-out. Legally meaningful when newsletter is enabled (GDPR /
    PIPL / Apple App Store all expect this)."""
    m = monetization or {}
    contact_email = ((m.get("social") or {}).get("contact_email") or "").strip() or "hello@bedtime.local"
...
def generate_terms_page(monetization: dict, base_url: str) -> str:
    """Terms of Service — covers usage rights, content license, AI disclosure,
    medical disclaimer (sleep content can be mistaken for medical advice)."""
    m = monetization or {}
    contact_email = ((m.get("social") or {}).get("contact_email") or "").strip() or "hello@bedtime.local"
    body = f"""
        <h2>1. 内容性质</h2>
...
def main():
    ...
    if _covers and _covers.available():
        ...
        # Per-episode 1400x1400 squares for iTunes per-item image (podcast apps
        # show these in now-playing and episode list views). Idempotent.
    ...
    # generate Privacy + Terms pages (legal hygiene for newsletter / GDPR / PIPL)
